	...
	m_states(0)
{
	...
	uint32_t sizes[]   = { 4, 1 };
	uint32_t types[]   = { GL_FLOAT, GL_BYTE };
	uint32_t len       = 6;
	uint32_t n         = 2;
...
void Bus::render()
{
	...
	m_vao.bind();
		glUniform4f(
			m_shader.getUniformLocation("off_color"),
			1,0,0,1
		);
		glUniform4f(
			m_shader.getUniformLocation("on_color"),
			0,1,0,1
		);
		glUniformMatrix4fv(
			m_shader.getUniformLocation("viewport"), 1, GL_FALSE, 
